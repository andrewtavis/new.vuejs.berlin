...
const upper = ref<null | SVGPathElement>(null)
const lower = ref<null | SVGPathElement>(null)

function setProgress(progress: number) {
  if (progress < 0) progress = 0
  if (progress > 1) progress = 1

  // we need the progress to be inverted for the animation to work
  progress = 1 - progress

  const upperLen = upper.value.getTotalLength()
  const lowerLen = lower.value.getTotalLength()

  upper.value.style.strokeDashoffset = upperLen * progress
  lower.value.style.strokeDashoffset = -lowerLen * progress

  upper.value.style.strokeDasharray = upperLen * progress
  lower.value.style.strokeDasharray = lowerLen * progress
}
...
onMounted(() => {
  if (props.progress === undefined) {
    nextTick(() => {
      setProgress(0)
      upper.value.classList.add('animated')
      lower.value.classList.add('animated')
   })
  } else {
